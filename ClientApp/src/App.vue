    ...
    <div class="app-container">
        ...
        <div class="sidebar" :class="{ 'sidebar-collapsed': isSidebarCollapsed }">
            <div class="sidebar-header">
                ...
                <div class="sidebar-actions" :class="{ 'collapsed-actions': isSidebarCollapsed }">
                    <button v-if="!isSidebarCollapsed" @click="handleRefresh" class="icon-btn"
            ...
            <div class="sidebar-content custom-scrollbar">
                ...
                <button @click="showFavorites" class="nav-item group" :class="{
                    ...
                    <div class="nav-icon"
                        :class="selectedPlatform?.address === 'favs' ? 'icon-fav-active' : 'icon-inactive'">
                ...
                <button @click="showHistory" class="nav-item group mb-spacer" :class="{
                    ...
                    <div class="nav-icon"
                        :class="selectedPlatform?.address === 'history' ? 'icon-hist-active' : 'icon-inactive'">
                        <History :size="18" />
                    </div>
                ...
                <template v-else-if="sortedPlatforms.length > 0">
                    ...
                    <button v-for="(p, idx) in sortedPlatforms" :key="`${p.address}-${idx}`" @click="fetchStreamers(p)"
                        ...
                        <div class="platform-img-wrapper" :class="{ 'mb-small': isSidebarCollapsed }">
                            <img :src="p.xinimg" :alt="p.title" class="platform-img"
                        ...
                        <div v-if="!isSidebarCollapsed" @click="(e) => togglePlatformFavorite(p.address, e)"
                            class="fav-toggle-btn" :class="{ 'visible': isPlatformFav(p.address) }"
                            :title="isPlatformFav(p.address) ? 'Remove Favorite' : 'Mark as Favorite'">
                            <Star :size="14"
                                :class="isPlatformFav(p.address) ? 'fav-icon-medium' : 'fav-icon-inactive'" />
                        </div>
                    </button>
                </template>
        ...
        <div class="main-area">
            ...
            <div class="top-pane" :class="{ 'expanded-top': selectedStreamer, 'flex-1': !selectedStreamer }">
                <VideoPlayer :url="selectedStreamer?.address || ''" :title="selectedStreamer?.title || ''"
                    @next="playNext" @prev="playPrev" :has-next="hasNext" :has-prev="hasPrev" />
            </div>

            <!-- Bottom: Streamers Grid -->
            <div class="bottom-pane"
                :class="{ 'shrunk-bottom': selectedStreamer, 'expanded-bottom': !selectedStreamer }">
                    ...
                    <div v-else-if="streamers.length > 0" class="streamer-grid">
                        <div v-for="(s, idx) in streamers" :key="`${s.address}-${idx}`" @click="selectStreamer(s)"
                            ...
                            <div class="card-img-wrapper">
                                <img :src="s.img" :alt="s.title" class="card-img"
                                ...
                                <button @click.stop="(e) => toggleStreamerFavorite(s, e)" class="card-fav-btn"
                                    ...
                                    <Star :size="16"
                                        :class="isStreamerFav(s.address) ? 'fav-icon-medium' : 'text-white'" />
                                </button>
                            </div>
                            <div class="card-info">
